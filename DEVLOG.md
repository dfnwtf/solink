...
Audio calls improvements.

Continued improving the audio calls system. Researched better signaling approaches, tested peer-to-peer connection stability, and refined the architecture. WebRTC in a browser environment has its quirks - handling ICE candidates, managing connection states, dealing with network changes. Spent time optimizing these edge cases for better reliability.

Fought with various bugs along the way - connection drops, audio glitches, state synchronization issues. Each fix revealed another edge case to handle.
...
Audio calls refinement.

Continued polishing the audio calls system. Improved the signaling protocol, refined the call state machine, and enhanced the WebRTC client wrapper. The challenge is making it work reliably across different network conditions while keeping encryption end-to-end. Tested various scenarios and edge cases to ensure stability.

More bug hunting - fixed race conditions in signaling, resolved audio routing issues, and patched connection recovery logic. The debugging process is tedious but necessary.
